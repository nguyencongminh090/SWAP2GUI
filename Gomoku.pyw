...
import connect
myInterface = Tk()
s = Canvas(myInterface, width=600, height=600, background= "#b69b4c")
s.pack()
...
def MouseClick(event):
    ...
    Click_Cord = str(Click_Cord[0] - 1) + ',' + str(15 - Click_Cord[1] )
...
def Score_Board(command=''):
    if Winner == None:
        if command == '':
            Turn_Text = s.create_text(width / 2, height - Frame_Gap + 15, text = "Turn = " + Turn, font = "Helvetica 25 bold", fill = Turn)
        else:
            Turn_Text = s.create_text(width / 2, height - Frame_Gap + 15, text = command, font = "Helvetica 25 bold", fill = Turn)
        return Turn_Text
    else:
...
def enmove(opening):
    create_circle(Board_X1 + Board_GapX * (X - 1), Board_Y1 + Board_GapY * (Y - 1), radius = Chess_Radius, fill = 'black')
    enmove = connect.testsw(opening, 'alphago.exe', 60*1000)
    if ',' in enmove:
        out = enmove.split(' ')
    else:
        return enmove
...
for f in range(Board_Size + 1):
    ...
    s.create_text(Board_X1 - Frame_Gap * 1.7, Board_Y1 + f * Board_GapY, text = 15-f, font = "Helvetica 10 bold", fill = "black")
    s.create_text(Board_X1 + f * Board_GapX, Board_Y1 - Frame_Gap * 1.7, text = chr(97+f), font = "Helvetica 10 bold", fill = "black")
...
while Winner == None:
    s.update()
    try:
        X = int(Click_Cord.split(',')[0]) + 1
        Y = 15 - int(Click_Cord.split(',')[1])
    except:
        continue
    ...
    if Picked:
        ...
        if Turn_Num == 3:
            Turn_Text = Score_Board(str(opening))
            s.delete(Turn_Text)
            Turn_Text = Score_Board(str(enmove(opening)))
        else:                              
            Turn_Text = Score_Board()
